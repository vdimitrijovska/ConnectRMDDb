using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;

namespace ConnectRMDDb.Entities
{
    public class Employee
    {
        public string work_on { get; set; }
        public string id_on { get; set; }
        public string id_name { get; set; }
        public string first_name { get; set; }
        public string last_name { get; set; }
        public string dep_no { get; set; }
        public string title_no { get; set; }
        public string card_no { get; set; }
        public string pin_code { get; set; }
        public string bmp_path { get; set; }
        public string door_only { get; set; }
        public string sex { get; set; }
        public string blood_type { get; set; }
        public string height { get; set; }
        public string weight { get; set; }
        public string birthday { get; set; }
        public string marriage { get; set; }
        public string born_place { get; set; }
        public string real_id_no { get; set; }
        public string nation_no1 { get; set; }
        public string nation_no2 { get; set; }
        public string passport1 { get; set; }
        public string passport2 { get; set; }
        public string army { get; set; }
        public string soldier { get; set; }
        public string current_status { get; set; }
        public string d_arrived { get; set; }
        public string job_level_no { get; set; }
        public string hire_no { get; set; }
        public string address1 { get; set; }
        public string address2 { get; set; }
        public string telephone1 { get; set; }
        public string telephone2 { get; set; }
        public string bbcall { get; set; }
        public string mobile { get; set; }
        public string fax { get; set; }
        public string e_mail { get; set; }
        public string em_contact { get; set; }
        public string call_no { get; set; }
        public string em_telephone { get; set; }
        public string motocycle { get; set; }
        public string car { get; set; }
        public string education_no { get; set; }
        public string religion_no { get; set; }
        public string families { get; set; }
        public string remark { get; set; }
        public string c1 { get; set; }
        public string c2 { get; set; }
        public string c3 { get; set; }
        public string c4 { get; set; }
        public string c5 { get; set; }
        public string c6 { get; set; }
        public string c7 { get; set; }
        public string c8 { get; set; }
        public string c9 { get; set; }
        public string c10 { get; set; }
        public string c11 { get; set; }
        public string c12 { get; set; }
        public string c13 { get; set; }
        public string c14 { get; set; }
        public string c15 { get; set; }
        public string d_in { get; set; }
        public string leave_date { get; set; }
    }
}